...
export function TradeCalculator() {
  ...
  const catalogForTab = useMemo(() => {
    ...
    return catalog.filter(
      (a) => a.kind === "player" && catalogPlayerHasSkillPosition(a.position, skill),
    );
  }, [catalog, catalogTab]);

  const filtered = useMemo(
    () =>
      catalogForTab.length ? filterTradeCatalogSuggestions(catalogForTab, query, effOpts) : [],
        ...
        <div className="flex flex-col gap-3 lg:flex-row lg:flex-wrap lg:items-center lg:justify-between lg:gap-x-4">
          <div className="flex flex-col gap-2 sm:flex-row sm:flex-wrap sm:items-center sm:gap-x-5 sm:gap-y-2">
            <div className="flex items-center gap-2 sm:gap-3 min-w-0">
              <label htmlFor="trade-league-size" className="text-sm font-medium text-dash-text/85 shrink-0">
                League size
              </label>
              <select
            ...
            <div className="flex items-center gap-2 sm:gap-3 min-w-0">
              <label htmlFor="trade-ppr" className="text-sm font-medium text-dash-text/85 shrink-0">
                PPR
              </label>
              <select
                id="trade-ppr"
            ...
            <div className="flex flex-wrap items-center gap-x-2 sm:gap-x-3 gap-y-1 min-w-0">
              <span id="trade-superflex-label" className="text-sm font-medium text-dash-text/85 shrink-0">
                Superflex
              </span>
              <button
                type="button"
