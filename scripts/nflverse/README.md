...
- The `player_id` column in those files is the **GSIS** id, which joins to Sleeper’s trimmed `gsis_id` from `players/nfl`.
...
| Script | Purpose |
| --- | --- |
| `npm run data:fantasy` | Writes **`player-fantasy-profile.json`** (nflverse + Sleeper id join) |
